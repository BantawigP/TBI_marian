...
export async function grantAccess(
  ...
  // CLONED APP: mock the grant-access edge function call (no backend)
  console.log('[mock] grantAccess called for', email, 'with role', role);
  return {
    success: true,
    message: `[Preview] Access invitation simulated for ${email} (no email sent in demo mode).`,
  };
}
 ...
 */
export async function claimAccess(_token: string): Promise<{ success: boolean; message: string }> {
  // CLONED APP: mock the claim-access edge function call (no backend)
  return {
    success: true,
    message: '[Preview] Access claim simulated (no backend in demo mode).',
  };
}
