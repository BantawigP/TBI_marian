...
type ReverificationInterval = 1 | 6 | 12;
...
const getNextDueInterval = (logs: CampaignLogRow[], nowIso: string): ReverificationInterval | null => {
  if (!logs.length) {
    return 1;
  }

  const sentIntervals = new Set(logs.map((entry) => entry.interval_months));
  const firstSentAt = logs[0].sent_at;
  const elapsedMonths = monthsBetween(firstSentAt, nowIso);
...
serve(async (req) => {
  ...
  for (const row of unverifiedRows ?? []) {
    ...
    const nextInterval = getNextDueInterval(logs, nowIso);
  ...
  for (const item of dueList) {
    ...
    const { error: sendError } = await supabase.functions.invoke("send-verification-email", {
      body: {
        to: item.email,
        subject: `Please verify your email (${item.interval}-month check)`,
    ...
    const { error: insertError } = await supabase.from("reverification_campaign_log").insert({
      email: item.email,
    ...
    if (insertError) {
      results.push({ email: item.email, interval: item.interval, sent: false, error: insertError.message });
      continue;
    }
